...
function welcome() {
  echo "This is the configuration script to start building OpenDelta."
...
if [ "$1" == "--clearPw" ]; then
  export ENV_OPENDELTA_KEYPWD=""
  export ENV_OPENDELTA_STOREPWD=""
fi
...
if [ "$1" == "--gpw" ]; then
  export ENV_OPENDELTA_KEYPWD="$2"
  export ENV_OPENDELTA_STOREPWD="$3"
fi

if [ -z $ENV_OPENDELTA_KEYPWD ]; then
  welcome
...
#  echo -en "\nKey      password: "
#  read -s -p "" THE_KEYPWD
#  export ENV_OPENDELTA_KEYPWD="$THE_KEYPWD"
#  export ENV_OPENDELTA_STOREPWD="$THE_STOREPWD"
  echo -en "\n"
  goodbye
fi
